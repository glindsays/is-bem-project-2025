    ...
    <div class="min-h-full">
        ...
        <main>
            <div class="mx-auto max-w-2xl px-4 py-6 sm:px-6 lg:px-8">
                ...
                <form method="POST" action="{{ route('tugas.update', $tugas->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="project_id" class="block text-sm font-medium text-gray-700">Project</label>
                        <select name="project_id" id="project_id" class="mt-1 block w-full rounded border-gray-300 shadow-sm">
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}" {{ $project->id == $tugas->project_id ? 'selected' : '' }}>
                                    {{ $project->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $tugas->title) }}" class="w-full border rounded px-3 py-2 @error('title') border-red-500 @enderror">
                        @error('title')
                    ...
                    <div>
                        ...
                        <input type="file" name="file" class="w-full border rounded px-3 py-2 @error('file_path') border-red-500 @enderror">
                        {{-- aku gatau gimana caranya naruh current file di kolom filenya ce TT --}}
                        @if ($tugas->file_path)
                            <p class="mt-2 text-sm text-gray-600">
                                Current File: 
                                <a href="{{ asset('storage/' . $tugas->file_path) }}" target="_blank" class="text-indigo-600 underline">
                                    {{ basename($tugas->file_path) }}
                                </a>
                            </p>
                            <div class="mt-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="delete_file" class="mr-2">
                                    <p class="text-sm text-gray-600">Delete current file
                                </label>
                            </div>
                        @endif
                        @error('file_path') <p class="text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    {{-- deskripsi --}}
                    <div>
                        <label class="block font-medium text-sm text-gray-700">Description</label>
                        <textarea name="description" class="w-full border rounded px-3 py-2 @error('description') border-red-500 @enderror">{{ old('description', $tugas->description) }}</textarea>
                        @error('description')
                            <div class="text-red-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="deadline" value="{{ old('deadline', $tugas->deadline->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2 @error('deadline') border-red-500 @enderror">
                        @error('deadline')
                            <div class="text-red-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full border rounded px-3 py-2 @error('status') border-red-500 @enderror">
                            @foreach (['to-do', 'in-progress', 'done'] as $status)
                                <option value="{{ $status }}" @selected(old('status', $tugas->status) === $status)>
                    ...
                    <div class="flex justify-between">
                        <a href="{{ route('tugas.index') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">Cancel</a>
